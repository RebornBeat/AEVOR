...
**Balance commitment + verifier sync ✅ DONE (Milestone 39) — cheap, not in the Merkle root.** Per-account balance changes ride in the block `StateDelta` and are bound by a `balance_commitment` (one BLAKE3 over the changes) in the attestation body. Verifiers check the shipped balances against the commitment (tamper-evident) and apply them to the ledger via HashMap writes (no Merkle), so a node that only *verified* a block stays balance-consistent and can *produce* correctly later (`verifier_stays_balance_consistent_on_fast_path`); a producer that ships/settles wrong balances is caught (hash mismatch at apply, or a re-executing validator recomputing the commitment → slashing); the signed commitment prevents equivocation. Measured: committing balances into the **Merkle root** costs +112–136% on the verify path (`bench_balance_in_root_overhead`, ≈ halving it); the `balance_commitment`+`StateDelta` path leaves the verify sweep unchanged at ~1.0–1.15M tx/s. Light-client balance proofs (Merkle inclusion for a specific account) remain an opt-in capability at that cost. *Next on the ledger:* network transport (gossip) for live multi-node, then F-E1 real TEE attestation.
