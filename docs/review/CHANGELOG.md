...
- **Verified:** aevor-execution 79 lib; node 56 lib + **24 e2e**; clippy 0 (`-p node --lib --tests`); benchmarks run with settlement inline.
- **Honest caveats (preserved):** aggregate 1M–100M TPS is an analytical projection (`lanes × per-lane`; single-core box); all test txs share the zero sender (a `new_simple` artifact, documented in the openers); balances are authoritative, deterministic, and rule-versioned but not yet committed *directly* into the state root (a scoped defense-in-depth enhancement for light-client balance proofs — deliberately not half-shipped here).
