...
> **Status.** Parts I–IX below are the standing *economic reference* — the design
> philosophy and the space of service/economic models. The **fee-market and
> validator-reward core is now finalized and implemented** in code; it is pinned
> concretely in "Finalized Fee-Market and Reward Mechanism" immediately after the
> executive summary. Where the prose and that section differ in specificity, the
> finalized section governs. AEVOR is launched as a **utility network for people
> to use**, not a funding-first token: there is no design dependence on a sale or
> speculation, transaction fees (not inflation) fund validators, and permissioned
> subnets can run entirely feeless.
...
## Finalized Fee-Market and Reward Mechanism

This section pins down the economics that are actually implemented and tested, in
concrete terms. It supersedes the abstract prose wherever they differ.

### Design stance: a utility network, not a funding vehicle

AEVOR is launched so that people can *use* it. The economics are built around
usage, not fundraising: the protocol never reads a token price, security is paid
for out of the fees that real usage generates rather than out of perpetual
inflation, and an operator who wants a zero-fee environment (a permissioned
enterprise or civic subnet) can run one. Nothing in the core mechanism depends on
a token sale, a speculative market, or artificial scarcity.

### One fee formula: `fee = total_gas × base_fee`

Every transaction, on mainnet or any subnet, is priced by the *same* formula the
VM's `GasMeter` uses — `fee = gas × price`. Two things feed it:

- **`total_gas` = execution gas + intrinsic gas.** Execution gas is metered by the
  VM per instruction and already includes the TEE execution premium, so
  privacy-protected work (which runs in a TEE) naturally costs more gas — the
  *privacy premium is priced in gas, not bolted on as a separate fee*. Intrinsic
  gas is a flat per-transaction base plus a **per-byte charge on the serialized
  transaction**, so a large post-quantum signature pays for the storage and
  bandwidth it imposes on every validator forever — the *bloat / post-quantum
  premium is priced per byte*.
- **`base_fee`** is the congestion-based price per gas unit (below).

Because both privacy and post-quantum bloat increase the resources a transaction
consumes — and therefore congestion — pricing them *through gas* is exactly right:
they raise a transaction's own cost and, in aggregate, they push the congestion
controller, which raises everyone's base fee until demand subsides.

### Congestion-based base fee (not token-price-based)

The base fee floats with **network congestion**, never with the token's market
price. Each block, it is updated by an EIP-1559-style rule in integer math (so it
is deterministic across validators):

```
next_base_fee = base_fee × (1 + adjustment_bps/10000 × (gas_used − target_gas)/target_gas)
```

floored at a configured minimum. When a block is **fuller than the target**, the
base fee **rises** (throttling demand, and paying validators more precisely when
the network is under load); when it is **emptier**, the base fee **falls** back
toward the floor (idle periods are cheap); at exactly the target it is unchanged.
`target_gas = block_gas_limit × target_utilization`. The tunable parameters
(base fee, floor, block gas budget, target utilization, adjustment rate) live in
the shared `FeeConfig`.

**Token price affects only the fiat display, not the protocol fee.** The native
fee for a given transaction under given congestion is identical whether the token
is worth $0.01 or $150; only the dollar conversion differs.

### Validator rewards from usage (no inflation required)

The fee a block collects is credited to the validator as its **block reward**.
Usage funds security directly: when the network is busy, fees (and therefore
rewards) rise; when it is idle, both fall. There is no need to mint new supply to
pay validators. This composes with the existing accountability layer —
attestation proves correct execution, and corruption is met with **slashing** of
the offender's stake (a graduated penalty; an invalid attestation is a 1% slash),
so rewards flow to honest work and misbehavior is penalized on the same ledger.

### Flexible subnet economics

A subnet chooses one of four fee modes, all expressed as a `FeeConfig` (or a fixed
policy) and enforced by the same pipeline as mainnet:

- **Mainnet's formula** — share `FeeConfig::default()` and inherit any future
  change to mainnet economics automatically.
- **Its own formula** — supply a custom `FeeConfig` (own base fee, target,
  adjustment rate).
- **Flat fee** — pin a fixed price with the congestion adjustment disabled
  (`adjustment_bps = 0`), for perfectly predictable costs.
- **None (feeless)** — `FeeConfig::feeless()`, for permissioned subnets that
  charge nothing.

### Measured, finalized numbers

Measured directly through the node at the default price of 1000 nano/gas
(1 AVR = 1e9 nanoAVR):
...
| Any transaction on a feeless subnet | — | 0 | 0 |

The post-quantum transaction costs **~11×** an Ed25519 one, entirely from its size.
Simulated congestion on a constrained subnet moves the base fee **1000 → 1634**
nano/gas across three full blocks and decays it back toward the floor
(**962** and falling) across the idle blocks that follow, while the validator's
cumulative reward climbs monotonically with usage. In fiat, the simple tx costs
**$0.000005** at $0.01/AVR up to **$0.073** at $150/AVR — the native fee never
changes; only the conversion does.

### dApp gas pricing

Application developers price against `total_gas`: their contract's instruction
cost (execution gas, with the TEE premium applied automatically if the dApp is
deployed at a privacy level that requires it) plus the intrinsic gas of the
calling transaction. A dApp on a feeless subnet runs at zero fee; a dApp on a
congestion-priced subnet pays the prevailing base fee, which is low when the
subnet is quiet and rises only under genuine load.
